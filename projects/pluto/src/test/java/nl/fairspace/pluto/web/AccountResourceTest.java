...
import nl.fairspace.pluto.JWTBuilder;
...
public class AccountResourceTest {
    ...
    public void exchangeTokensAllowsForConsecutiveRequests() throws Exception {
        ...
        ResponseEntity<Map> tokenData = restTemplate.exchange("http://localhost:" + port + "/api/account/tokens", HttpMethod.POST, request, Map.class);
    ...
    @Test
    public void exchangeInvalidTokenFails() throws Exception {
        JWT accessToken = new JWTBuilder().signWith(keyId, privateKey).authorities(Collections.singletonList("user-workspace")).expires(new Date()).build();
        JWT refreshToken = new JWTBuilder().signWith(keyId, privateKey).build();
        ...
        ResponseEntity<Map> tokenData = restTemplate.exchange("http://localhost:" + port + "/api/account/tokens", HttpMethod.POST, request, Map.class);
        assertEquals(400, tokenData.getStatusCodeValue());
    }
