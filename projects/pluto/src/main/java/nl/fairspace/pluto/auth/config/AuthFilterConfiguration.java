...
public class AuthFilterConfiguration {
    ...
    public AuthFilterConfiguration(
            OidcConfiguration oidcConfiguration,
            AppSecurityUrlConfig urlConfig,
            OAuthFlow oAuthFlow,
    ...
    public GlobalFilter sessionAuthenticationFilter() {
        return (exchange, chain) ->
                new SessionAuthenticationFilter(jwtTokenValidator, oAuthFlow).filter(exchange, chain);
    }
    ...
    public GlobalFilter authorizedCheckAuthenticationFilter() {
        return (exchange, chain) -> new AuthorizedCheckAuthenticationFilter(oidcConfiguration.getRequiredAuthority())
    ...
    public GlobalFilter anonymousCheckAuthenticationFilter() {
        return (exchange, chain) -> {
            if (Arrays.stream(urlConfig.getPermitAll())
                    .anyMatch(s -> s.equals(exchange.getRequest().getPath().toString()))) {
    ...
    public GlobalFilter handleFailedAuthenticationFilter() {
        return (exchange, chain) ->
                new HandleFailedAuthenticationFilter(new AuthorizationFailedHandler()).filter(exchange, chain);
    }
}
