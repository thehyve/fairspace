...
public class ShapeUtils {
    ...
    public static Optional<Resource> getClassShapeForClass(Resource type, Model vocabulary) {
        return vocabulary
                .listSubjectsWithProperty(SHACLM.targetClass, type)
                .nextOptional()
                .or(() -> Optional.of(type.inModel(vocabulary))
                        .filter(shape -> shape.hasProperty(RDF.type, SHACLM.NodeShape)
                                && shape.hasProperty(RDF.type, RDFS.Class)));
    }
