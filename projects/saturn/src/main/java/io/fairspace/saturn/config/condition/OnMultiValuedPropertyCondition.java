...
public class OnMultiValuedPropertyCondition extends SpringBootCondition {
    ...
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata ) {
        MergedAnnotation<ConditionalOnMultiValuedProperty> conditionalMultiValuedPropertyAnnotation = metadata.getAnnotations()
                .get( ConditionalOnMultiValuedProperty.class.getName() );
        ...
        Binder.get( context.getEnvironment() )
                .bind( prefix + "." + name, Bindable.ofInstance( values ) );

        String valueToCheck = conditionalMultiValuedPropertyAnnotation.getString( "havingValue" );
        boolean valueExists = values.stream()
                .anyMatch( valueToCheck::equals );
        if ( valueExists ) {
