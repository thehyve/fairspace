...
public class ReadableMetadataServiceTest {
    ...
    public void setUp() {
        ds = createTxnMem();
        api = new ReadableMetadataService(new RDFLinkSimple(ds), createURI(GRAPH), createURI(userVocabularyURI));
    }
    ...
    public void testTripleLimit() {
        api = new ReadableMetadataService(new RDFLinkSimple(ds), createURI(GRAPH), createURI(userVocabularyURI), 1);
        executeWrite(ds, () -> ds.getNamedModel(GRAPH).add(STMT1).add(STMT2));
