...
const ignoredProperties = ['filename', 'basename', 'displayname', 'name', 'type', 'iri', 'ownedBy', 'ownedByCode', 'access', 'canRead', 'canWrite', 'canManage', 'canDelete', 'canUndelete', 'canUnpublish', 'accessMode', 'isreadonly', 'userPermissions', 'availableStatuses', 'workspacePermissions', 'availableAccessModes', 'status', 'getcreated', 'getcontenttype', 'mime', 'etag', 'getetag', 'iscollection', 'supported-report-set', 'resourcetype', 'getlastmodified', 'getcontentlength', 'size', 'metadataLinks', 'version'];
...
const mapFileProperties = (data: any = {}, users: User[] = []): Map<string, LabelValueProperty> => {
    ...
    const propertiesToDisplay = Object.keys(data).filter(k => !ignoredProperties.includes(k) && !Object.keys(defaultProperties).includes(k));
...
const useExternalStorageMetadata = (path: string, fileAPI: FileAPI) => {
    ...
    const fetchLinkedMetadataEntities = (subjects = []) => {
        ...
        MetadataAPI.getForAllSubjects(subjects).then(results => {
            if (results) {
        ...
        }).catch(() => null).finally(() => setLinkedMetadataEntitiesLoading(false));
    };
    ...
    const fetchMetadata = () => {
        setLoading(true);
        fileAPI.stat(path, false, true).then(results => {
