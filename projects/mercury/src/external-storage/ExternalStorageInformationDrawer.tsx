...
const ExternalMetadataCard = (props: ExternalMetadataCardProperties) => {
    ...
    const renderProperty = (data: Map<string, DisplayProperty>, key: string) => data[key] && data[key].value != null && data[key].value !== "" && <ListItem disableGutters key={key}>
        <FormControl>
            <FormLabel>{data[key].label || key}</FormLabel>
            <FormGroup>
                <ListItemText primary={data[key].value} style={{
                    whiteSpace: 'pre-line'
                }} />
            </FormGroup>
        </FormControl>
    ...
    const renderLinkProperties = (data: Map<string, DisplayProperty>, key: string) => data[key] && data[key].length > 0 && <ListItem disableGutters key={key}>
        <FormControl>
            <FormLabel>{key}</FormLabel>
            <FormGroup>
                {data[key].map(value => <LinkedDataLink uri={value.id} key={value.id}>
    ...
    const renderCardContent = () => {
        ...
        return <List>
            ...
            {linkedMetadataEntitiesLoading ? <div>Loading linked metadata entities...</div> : Object.keys(linkedMetadataEntities).map(k => renderLinkProperties(linkedMetadataEntities, k))}
        </List>;
    };
    ...
    return <Card className={classes.card}>
        <CardHeader titleTypographyProps={{
            variant: 'h6'
        }} title={title} avatar={avatar} style={{
            wordBreak: 'break-word'
        }} action={<IconButton onClick={toggleExpand} aria-expanded={expanded} aria-label="Show more" className={expanded ? classes.expandOpen : ''} size="medium">
            <ExpandMore />
        </IconButton>} />
...
export const ExternalStorageInformationDrawer = (props: ExternalStorageInformationDrawerProperties) => {
    ...
    if (paths.length === 0 && !selected) {
        return atLeastSingleRootFileExists ? <EmptyInformationDrawer message="Select a file or a folder to display its metadata" /> : <></>;
    }

    return paths.map((p, index) => <ExternalMetadataCard key={p} title={`Metadata for ${getPathToDisplay(p)}`} forceExpand={index === paths.length - 1} path={p} storage={storage} />);
};
