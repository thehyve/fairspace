...
function shallowRender(path = '', storage = 'xyz', location) {
  ...
  return shallow(<ExternalStoragePage match={match} location={location || {
    pathname: path
  }} externalStorages={storages} classes={{}} />);
}
...
describe('ExternalStoragePage', () => {
  ...
  it('renders an external storage browser and information drawer', () => {
    ...
    expect(externalStorageBrowserProps.storage.label).toBe('Some external storage'); // const informationDrawerProps = wrapper.find(InformationDrawer).first().props(); // TODO
  });
