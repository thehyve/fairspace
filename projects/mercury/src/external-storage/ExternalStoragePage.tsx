...
export const ExternalStoragePage = (props: ExternalStoragePageProperties) => {
  ...
  return <ExternalStorageBreadcrumbsContextProvider storage={storage}>
            <BreadCrumbs additionalSegments={breadcrumbSegments} />
            {isSearchAvailable && <Grid container justifyContent="space-between" spacing={1}>
            ...
            <Grid container spacing={1}>
                <Grid item className={classes.centralPanel}>
                    <ExternalStorageBrowser selection={selection} preselectedFile={preselectedFile} storage={storage} pathname={location.pathname} setBreadcrumbSegments={setBreadcrumbSegments} setAtLeastSingleRootFileExists={setAtLeastSingleRootFileExists} />
                </Grid>
                <Grid item className={classes.sidePanel}>
                    <ExternalStorageInformationDrawer atLeastSingleRootFileExists={atLeastSingleRootFileExists} path={getRelativePath(location.pathname, storage.name)} selected={selection.selected} storage={storage} />
                </Grid>
            </Grid>
