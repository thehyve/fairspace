...
import UploadsContext, { UPLOAD_STATUS_ERROR, UPLOAD_STATUS_FINISHED, UPLOAD_STATUS_IN_PROGRESS, UPLOAD_STATUS_INITIAL } from "./UploadsContext";
...
export const UploadProgressComponent = ({
  classes
}) => {
  const {
    uploads,
  ...
  const cardHeaderAction = <IconButton onClick={toggleExpand} className={classnames(classes.expand, {
    [classes.expandOpen]: expanded
  })} aria-expanded={expanded} aria-label="Show more" size="medium">
            <ExpandMore />
        </IconButton>;
  ...
  const renderFolderUploadName = (upload, folderName) => {
    ...
    return <Typography variant="body2" className={classes.wrapIcon} component="span">
  ...
  const renderFileUploadName = upload => <Typography variant="body2" className={classes.wrapIcon}>
  ...
  const renderUploadList = () => <div style={{
    overflowX: 'auto'
  }}>
            {uploads.length > 0 && <Table>
                    <TableHead>
                        <TableRow>
                    ...
                    <TableBody>
                        {uploads.map(upload => <TableRow key={upload.id}>
                                <TableCell>{renderUploadName(upload)}</TableCell>
                                <TableCell width={100}>{progress(upload)}</TableCell>
                                <TableCell width={40} padding="none">
                                    {upload.status === UPLOAD_STATUS_ERROR && <IconButton aria-label="remove" onClick={() => removeUpload(upload)} size="medium">
