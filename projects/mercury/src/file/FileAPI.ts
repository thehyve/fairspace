...
import {decodeHTMLEntities, encodePath, generateUniqueFileName, getFileName, joinPaths, joinPathsAvoidEmpty} from "./fileUtils";
...
const includeDetails = {...defaultOptions,
    details: true};
...
class FileAPI {
    ...
    stat(path, showDeleted = false, includeMetadataLinks = false) {
        ...
        if (includeMetadataLinks) {
            options.headers = {...options.headers,
                "With-Metadata-Links": true};
        }

        return this.client().stat(path, options).then(result => this.mapToFile(result.data));
    }
   ...
   */
    list(path, showDeleted = false): File[] {
        ...
        return this.client().getDirectoryContents(path, options).then(result => result.data.sort(comparing(compareBy('type'), compareBy('filename'))).map(this.mapToFile));
    }
   ...
   */
    createDirectory(path, options = defaultOptions) {
        return this.client().createDirectory(path, options).catch(e => {
            if (e && e.response) {
   ...
   */
    upload(file, destinationFilename, destinationPath, deleteExistingBlob = false) {
        ...
        if (deleteExistingBlob) {
            options.headers = {...options.headers,
                "Delete-Existing-Blob": true};
        }

        return this.client().putFileContents(`${destinationPath}/${destinationFilename}`, file, options).catch(e => {
            if (e && e.response) {
                // eslint-disable-next-line default-case
                switch (e.response.status) {
                    case 403:
    ...
    uploadMulti(destinationPath, files: File[], maxFileSizeBytes: number, onUploadProgress = () => {}) {
        ...
        return this.uploadClient().customRequest(destinationPath, requestOptions).catch(e => {
            if (e && e.response) {
                // eslint-disable-next-line default-case
                switch (e.response.status) {
                    case 413:
   ...
   */
    delete(path, showDeleted = false) {
        ...
        return this.client().deleteFile(path, options).catch(e => {
            if (e && e.response) {
                // eslint-disable-next-line default-case
                switch (e.response.status) {
                    case 403:
   ...
   */
    move(source, destination) {
        ...
        // to the
        return this.client().moveFile(source, destination, defaultOptions).catch(e => {
            if (e && e.response) {
                // eslint-disable-next-line default-case
                switch (e.response.status) {
                    case 400:
   ...
   */
    copy(source, destination) {
        ...
        return this.client().copyFile(source, destination, defaultOptions).catch(e => {
            if (e && e.response) {
                // eslint-disable-next-line default-case
                switch (e.response.status) {
                    case 403:
   ...
   */
    movePaths(filePaths, destinationDir) {
        return this.uniqueDestinationPaths(filePaths, destinationDir).then(mapping => Promise.all(mapping.map(([src, dst]) => this.move(src, dst))));
    }
   ...
   */
    copyPaths(filePaths, destinationDir) {
        return this.uniqueDestinationPaths(filePaths, destinationDir).then(mapping => Promise.all(mapping.map(([src, dst]) => this.copy(src, dst)))).catch(e => {
            if (e && e.response) {
                // eslint-disable-next-line default-case
                switch (e.response.status) {
                    case 403:
   ...
   */
    uniqueDestinationPaths(filePaths, destinationDir) {
        return this.list(destinationDir).then(files => files.map(f => f.basename)).then(usedNames => filePaths.map(sourceFile => {
   ...
   */
    ...
    post(path, data, showDeleted = false) {
        ...
        return this.client().customRequest(path, requestOptions).catch(handleHttpError("Error performing POST request"));
    }
