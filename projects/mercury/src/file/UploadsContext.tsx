...
export const showCannotOverwriteDeletedError = (filesLength: number) => {
    const errorMessage = filesLength === 1 ? "File or folder with this name already exists and was marked as deleted.\n" + "Please delete the existing one permanently, undelete it first\n" + "to be able to overwrite it or choose a unique name." : "Some of the uploaded files or folders already exist and were marked as deleted.\n" + "Please delete the existing ones permanently, undelete them first \n" + "to be able to overwrite them or choose unique names.";
    ErrorDialog.showError("Cannot overwrite deleted file or folder", errorMessage);
};
export const showInvalidFilenames = invalidFilenames => {
    ErrorDialog.showError('Invalid file name', <span>
...
export const showFileTooLarge = (size: string) => {
    ErrorDialog.showError('File too large', <span>
...
export const UploadsProvider = ({
    ...
    const handleGetMaxFileSize = async () => getServerConfig().then((response: ConfigResponse) => {
        if (response) {
    ...
    const updateSpecificUpload = (selected, updateFunc) => setUploads(currentUploads => currentUploads.map(upload => {
    ...
    const setStateForUpload = (selected, newState) => updateSpecificUpload(selected, upload => ({...upload,
        status: newState}));
    ...
    const startUpload = (upload: FileUpload) => {
        const newUpload = {...upload,
            status: UPLOAD_STATUS_INITIAL,
            progress: 0};
        enqueueUploads(newUpload);
        const invalidFilenames = upload.files.map(file => {
        ...
        const onUploadProgress = progressEvent => updateSpecificUpload(newUpload, u => ({...u,
            progress: progressEvent.loaded * 100 / progressEvent.total}));

        setStateForUpload(newUpload, UPLOAD_STATUS_IN_PROGRESS);
        return fileApi.uploadMulti(newUpload.destinationPath, newUpload.files, maxFileSizeBytes, onUploadProgress).then(() => {
    ...
    return <UploadsContext.Provider value={{
        uploads,
        startUpload,
        removeUpload,
        maxFileSize
