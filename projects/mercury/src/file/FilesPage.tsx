...
export const FilesPage = (props: FilesPageProperties) => {
  ...
  const getMetadataSearchRedirect = () => `${getMetadataViewsPath()}?${queryString.stringify({
    view: RESOURCES_VIEW,
  ...
  const breadcrumbSegments = collection.name ? pathSegments.map((segment, idx) => ({
  ...
  const showMetadataSearchButton: boolean = currentUser && currentUser.canViewPublicMetadata && views && views.some(v => v.name === RESOURCES_VIEW) && !isOpenedPathDeleted && collection.iri;
  return <CollectionBreadcrumbsContextProvider>
            ...
            <Grid container justifyContent="space-between" spacing={1}>
                <Grid item className={classes.topBar}>
                    <Grid container>
                        <Grid item xs={6}>
                            <SearchBar placeholder={`Search in ${openedPath.substring(openedPath.lastIndexOf('/') + 1)}`} onSearchChange={handleTextSearch} />
                        </Grid>
                        {showMetadataSearchButton && <Grid item container xs={4} justifyContent="flex-end">
                                <Grid item>
                                    <Button variant="text" color="primary" href={getMetadataSearchRedirect(RESOURCES_VIEW)}>
                        ...
                        <Grid item xs={2} className={classes.topBarSwitch}>
                            <FormControlLabel control={<Switch color="primary" checked={showDeleted} onChange={() => setShowDeleted(!showDeleted)} disabled={isOpenedPathDeleted} />} label="Show deleted" />
                        </Grid>
                    </Grid>
                </Grid>
            </Grid>
            <Grid container spacing={1}>
                <Grid item className={classes.centralPanel}>
                    <FileBrowser data-testid="file-browser" openedCollection={collection} openedPath={openedPath} isOpenedPathDeleted={isOpenedPathDeleted} loading={loading} error={error} selection={selection} preselectedFile={preselectedFile} showDeleted={showDeleted} />
                </Grid>
                <Grid item className={classes.sidePanel}>
                    <CollectionInformationDrawer setBusy={setBusy} path={path} selectedCollectionIri={collection.iri} showDeleted={showDeleted} />
                </Grid>
            </Grid>
...
const ParentAwareFilesPage = (props: ParentAwareFilesPageProperties) => {
  const {
    ...
    loading,
    refresh
  } = useAsync(() => LocalFileAPI.stat(props.openedPath, true), [props.openedPath]);
  useEffect(() => {
    refresh();
  ...
  return <FilesPage isOpenedPathDeleted={isOpenedPathDeleted} loading={loading && props.loading} error={error && props.error} {...props} />;
};
...
const ContextualFilesPage = (props: ContextualFilesPageProperties) => {
  ...
  const {
    collectionName,
    openedPath
  } = getPathInfoFromParams(params);
  const collection = collections.find(c => c.name === collectionName) || {};
  return showDeleted ? <ParentAwareFilesPage collection={collection} openedPath={openedPath} loading={loading} error={error} showDeleted={showDeleted} setShowDeleted={setShowDeleted} currentUser={currentUser} views={views} {...props} /> : <FilesPage collection={collection} openedPath={openedPath} loading={loading} error={error} showDeleted={showDeleted} setShowDeleted={setShowDeleted} currentUser={currentUser} views={views} {...props} />;
};
