...
describe('FileAPI', () => {
    ...
    describe('Moving', () => {
        ...
        it('should result in a clear error on 400 response', () => {
            ...
            return expect(LocalFileAPI.move('/test', 'special-characters')).rejects.toThrow(
    ...
    describe('Copying', () => {
        ...
        it('should result in a clear error on 409 response', () => {
            ...
            return expect(LocalFileAPI.copy('/test', 'special-characters')).rejects.toThrow(
                /destination can not be copied to/
            );
        });
    ...
    describe('Deleting', () => {
        it('should result in a clear error on 403 response', () => {
            LocalFileAPI.client = () => ({
                deleteFile: jest.fn(() => Promise.reject({response: {status: 403}}))
            });
        ...
        it('deletes files permanently', async () => {
            ...
            expect(deleteFile).toHaveBeenCalledTimes(1);
            expect(deleteFile).toHaveBeenCalledWith('/src', {
                headers: {'X-Requested-With': 'XMLHttpRequest', 'Show-Deleted': 'on'},
                withCredentials: true
            });
        });
    });
    ...
    describe('Creating directory', () => {
        ...
        it('should result in clear error on 405 response', () => {
            LocalFileAPI.client = () => ({
                createDirectory: () => Promise.reject({response: {status: 405}})
            });
    ...
    describe('Showing history', () => {
        it('shows file history', async () => {
            ...
            const stat = jest.fn(() => Promise.resolve({data: file}));
        ...
        it('shows only limited number of file versions', async () => {
            ...
            const stat = jest.fn(() => Promise.resolve(Promise.resolve({data: file})));
            LocalFileAPI.client = () => ({stat});
