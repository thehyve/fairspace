...
describe('FileAPI', () => {
  ...
  describe('Moving', () => {
    ...
    it('should result in a clear error on 400 response', () => {
      const moveFile = jest.fn(() => Promise.reject({
        response: {
          status: 400
        }
      }));
      ...
      return expect(LocalFileAPI.move("/test", "special-characters")).rejects.toThrow(/contains special characters/);
    });
  });
  describe('Copying', () => {
    it('should result in a clear error on 403 response', () => {
      const copyFile = jest.fn(() => Promise.reject({
        response: {
          status: 403
        }
      }));
    ...
    it('should result in a clear error on 409 response', () => {
      ...
      return expect(LocalFileAPI.copy("/test", "special-characters")).rejects.toThrow(/destination can not be copied to/);
    });
  ...
  describe('Deleting', () => {
    it('should result in a clear error on 403 response', () => {
      LocalFileAPI.client = () => ({
        deleteFile: jest.fn(() => Promise.reject({
          response: {
            status: 403
          }
        }))
      ...
      return expect(LocalFileAPI.delete('path')).rejects.toThrow(/Only admins can delete/);
    });
  ...
  describe('uniqueDestinationPaths', () => {
    it('generates unique names', async () => {
      ...
      expect(result).toEqual([['/src/file.ext', '/dst/file (3).ext'], ['/src/file (2).ext', '/dst/file (2) (1).ext']]);
    });
  ...
  describe('Creating directory', () => {
    it('should result in clear error on 400 response', () => {
      LocalFileAPI.client = () => ({
        createDirectory: () => Promise.reject({
          response: {
            status: 400
          }
        })
    ...
    it('should result in clear error on 405 response', () => {
      ...
      return expect(LocalFileAPI.createDirectory("/test")).rejects.toThrow(/already exists/);
    });
  });
