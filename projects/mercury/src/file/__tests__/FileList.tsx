...
const wrap = element => <ThemeProvider theme={theme}>
        {element}
    </ThemeProvider>;
...
describe('FileList', () => {
  ...
  it('renders view with deleted column in "show deleted" mode', () => {
    const {
      queryByText
    } = render(wrap(<FileList files={files} showDeleted />));
    expect(queryByText(/empty directory/i)).not.toBeInTheDocument();
  ...
  it('calls onPathCheckboxClick when the checkbox container is clicked', () => {
    ...
    const {
      getByTestId
    } = render(wrap(<FileList onPathCheckboxClick={onPathCheckboxClick} selectionEnabled files={files} />));
  ...
  it('does not render the checkbox when selection is disabled', () => {
    const {
      queryByTestId
    } = render(wrap(<FileList selectionEnabled={false} files={files} />));
