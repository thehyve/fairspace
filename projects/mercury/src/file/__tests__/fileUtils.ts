// @ts-nocheck
import { decodePath, encodePath, generateUniqueFileName, getBaseNameAndExtension, getFileName, getParentPath, getPathFromIri, getPathInfoFromParams, joinPathsAvoidEmpty, redirectLink } from "../fileUtils";
...
describe('getBaseNameAndExtension', () => {
  it('should return the expected file base name and extension', () => {
    expect(getBaseNameAndExtension()).toEqual({
      baseName: '',
      extension: ''
    });
    expect(getBaseNameAndExtension(undefined)).toEqual({
      baseName: '',
      extension: ''
    });
    expect(getBaseNameAndExtension(null)).toEqual({
      baseName: '',
      extension: ''
    });
    expect(getBaseNameAndExtension('name.ext')).toEqual({
      baseName: 'name',
      extension: '.ext'
    });
    ...
    expect(getBaseNameAndExtension('name.')).toEqual({
      baseName: 'name',
      extension: '.'
    });
    expect(getBaseNameAndExtension('name')).toEqual({
    ...
    expect(getBaseNameAndExtension('.hidden')).toEqual({
      baseName: '.hidden',
      extension: ''
    });
  });
...
describe('redirectLink', () => {
  it('gets a file redirection link', () => {
    expect(redirectLink('http://localhost:8080/api/webdav/collection%202021-05-27_13_39-0/dir_1/coffee_139.jpg', FILE_URI)).toEqual('/collections/collection%202021-05-27_13_39-0/dir_1?selection=%2Fcollection%202021-05-27_13_39-0%2Fdir_1%2Fcoffee_139.jpg');
  });
  it('gets a directory redirection link', () => {
    expect(redirectLink('http://localhost:8080/api/webdav/collection%202021-05-27_13_39-0/dir_1/', DIRECTORY_URI)).toEqual('/collections/collection%202021-05-27_13_39-0/dir_1');
  });
  it('gets a file redirection link for external storage', () => {
    ...
    expect(redirectLink('http://localhost:8080/api/webdav/collection%202021-05-27_13_39-0/dir_1/coffee_139.jpg', FILE_URI, storage)).toEqual('/external-storages/test/collection%202021-05-27_13_39-0/dir_1?selection=%2Fcollection%202021-05-27_13_39-0%2Fdir_1%2Fcoffee_139.jpg');
  });
  it('gets a directory redirection link for external storage', () => {
    ...
    expect(redirectLink('http://localhost:8080/api/webdav/collection%202021-05-27_13_39-0/dir_1', DIRECTORY_URI, storage)).toEqual('/external-storages/test/collection%202021-05-27_13_39-0/dir_1');
  });
});