...
describe('FileBrowser', () => {
  const renderWithProviders = children => render(<UploadsProvider>
            {children}
        </UploadsProvider>);
  ...
  it('renders proper view', () => {
    const {
      queryByTestId
    } = renderWithProviders(<ThemeProvider theme={theme}>
  ...
  it('show error when no open collection is provided', () => {
    const {
      getByText
    } = renderWithProviders(<FileBrowser {...initialProps} />);
  ...
  it('show no open collection error when no collection is provided even when another error is given', () => {
    const {
      getByText
    } = renderWithProviders(<FileBrowser {...initialProps} error="some error" />);
    expect(getByText(/collection does not exist/i)).toBeInTheDocument();
  });
  it('cleans up listener after unmount', () => {
    ...
    const {
      unmount
    } = renderWithProviders(<FileBrowser {...initialProps} history={{
      listen: () => cleanupFn
    }} />);
    unmount();
