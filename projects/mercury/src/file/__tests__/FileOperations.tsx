...
describe('FileOperations', () => {
    ...
    const renderFileOperations = (clipboard, fileActions, openedPath) => shallow(<FileOperations classes={{}} paste={() => Promise.resolve()} files={[{
        filename: 'a'
    }]} selectedPaths={['a']} clipboard={clipboard} fetchFilesIfNeeded={() => {}} getDownloadLink={() => {}} refreshFiles={refreshFiles} clearSelection={clearSelection} fileActions={fileActions} openedPath={openedPath} isWritingEnabled />);
    ...
    it('should enable all buttons after successful file operation', () => {
        return clickHandler('Paste')({
            stopPropagation: () => {}
        }).then(() => {
            wrapper.find(IconButton).not('[download]').not('[aria-label="Show history"]').forEach(b => {
    ...
    it('should clear selection and refresh files after all successful file operations', () => {
        ...
        const paste = () => clickHandler('Paste')({
            stopPropagation: () => {}
        });

        const deleteFile = () => wrapper.find('[aria-label="Delete"]').parent().prop("onClick")();

        return Promise.all([createDir, deleteFile, paste].map(op => {
            refreshFiles.mockReset();
            clearSelection.mockReset();
            return op().then(() => {
    ...
    describe('undelete button', () => {
        ...
        it('should not be shown if not in showDeleted mode', () => {
            const render = fileActions => shallow(<FileOperations classes={{}} paste={() => Promise.resolve()} files={[{
                filename: 'a'
            }]} selectedPaths={[]} fetchFilesIfNeeded={() => {}} getDownloadLink={() => {}} refreshFiles={refreshFiles} clearSelection={clearSelection} fileActions={fileActions} openedPath={{}} isWritingEnabled clipboard={emptyClipboard} showDeleted={false} />);
        ...
        it('should be disabled if no file selected', () => {
            const render = fileActions => shallow(<FileOperations classes={{}} paste={() => Promise.resolve()} files={[{
                filename: 'a'
            }]} selectedPaths={[]} fetchFilesIfNeeded={() => {}} getDownloadLink={() => {}} refreshFiles={refreshFiles} clearSelection={clearSelection} fileActions={fileActions} openedPath={{}} isWritingEnabled clipboard={emptyClipboard} showDeleted />);
        ...
        it('should be disabled if non-deleted file selected', () => {
            const render = fileActions => shallow(<FileOperations classes={{}} paste={() => Promise.resolve()} files={[{
                filename: 'a'
            }]} selectedPaths={['a']} fetchFilesIfNeeded={() => {}} getDownloadLink={() => {}} refreshFiles={refreshFiles} clearSelection={clearSelection} fileActions={fileActions} openedPath={{}} isWritingEnabled clipboard={emptyClipboard} showDeleted />);
    ...
    describe('show history button', () => {
        ...
        it('should be disabled if no file selected', () => {
            const render = fileActions => shallow(<FileOperations classes={{}} paste={() => Promise.resolve()} files={[{
                filename: 'a',
                type: 'file'
            }]} selectedPaths={[]} fetchFilesIfNeeded={() => {}} getDownloadLink={() => {}} refreshFiles={refreshFiles} clearSelection={clearSelection} fileActions={fileActions} openedPath={{}} isWritingEnabled clipboard={emptyClipboard} />);
        ...
        it('should be enabled if one file selected', () => {
            const render = fileActions => shallow(<FileOperations classes={{}} paste={() => Promise.resolve()} files={[{
                filename: 'a',
                type: 'file'
            }]} selectedPaths={['a']} fetchFilesIfNeeded={() => {}} getDownloadLink={() => {}} refreshFiles={refreshFiles} clearSelection={clearSelection} fileActions={fileActions} openedPath={{}} isWritingEnabled clipboard={emptyClipboard} />);
