...
const getConflictingFolders: string[] = (newFiles, existingFolderNames) => {
    const droppedFolderNames = new Set(newFiles.filter(f => splitPathIntoArray(f.path).length > 1).map(f => splitPathIntoArray(f.path)[0]));
    return Array.from(droppedFolderNames).filter(f => existingFolderNames.includes(f));
};

const getConflictingFiles: string[] = (newFiles, existingFileNames) => newFiles.filter(f => existingFileNames.includes(f.path)).map(c => c.name);
...
export const FileBrowser = (props: FileBrowserProperties) => {
    ...
    useEffect(() => {
        ...
        return historyListener; // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [history]);
    ...
    if (!collectionExists) {
        return <MessageDisplay message="This collection does not exist or you don't have sufficient permissions to view it." variant="h6" noWrap={false} />;
    }
    ...
    const renderOverwriteConfirmationMessage = () => <Typography variant="body2" component="span">
        {overwriteFolderCandidateNames.length > 1 && <span>
        ...
        {overwriteFileCandidateNames.length > 1 && <span>
                    Files: <em>{overwriteFileCandidateNames.join(', ')}</em> already exist.<br />
        </span>}
        {overwriteFileCandidateNames.length === 1 && <span>
                    File <em>{overwriteFileCandidateNames[0]}</em> already exists.<br />
        </span>}
        {overwriteFolderCandidateNames.length + overwriteFileCandidateNames.length === 1 ? <span>Do you want to <b>overwrite</b> it?</span> : <span>Do you want to <b>overwrite</b> them?</span>}
    </Typography>;

    const renderOverwriteConfirmation = () => <ConfirmationDialog open title="Warning" content={renderOverwriteConfirmationMessage()} dangerous agreeButtonText="Overwrite" onAgree={() => {
        startUpload(currentUpload).then(refreshFiles);
        handleCloseUpload();
    }} onDisagree={handleCloseUpload} onClose={handleCloseUpload} />;

    const renderFileOperations = () => <div style={{
        marginTop: 8
    }}>
        <FileOperations selectedPaths={selection.selected} files={files} openedPath={openedPath} isWritingEnabled={isWritingEnabled} showDeleted={showDeleted} fileActions={fileActions} clearSelection={selection.deselectAll} refreshFiles={refreshFiles} uploadFolder={uploadFolder} uploadFile={uploadFile} maxFileSize={maxFileSize} />
    </div>;

    return <div data-testid="files-view" className={classes.container}>
        <div {...getRootProps()} className={`${classes.dropzone} ${isDragActive && classes.activeStyle} ${isDragAccept && classes.acceptStyle} ${isDragReject && classes.rejectStyle}`}>
            <input {...getInputProps()} {...isFolderUpload && {
                webkitdirectory: ""
            }} />
            <FileList selectionEnabled={openedCollection.canRead} files={files.map(item => ({...item,
                selected: selection.isSelected(item.filename)}))} onPathCheckboxClick={path => selection.toggle(path.filename)} onPathHighlight={handlePathHighlight} onPathDoubleClick={handlePathDoubleClick} onAllSelection={shouldSelectAll => shouldSelectAll ? selection.selectAll(files.map(file => file.filename)) : selection.deselectAll()} showDeleted={showDeleted} preselectedFile={preselectedFile} />
...
const ContextualFileBrowser = (props: ContextualFileBrowserProperties) => {
    ...
    return <FileBrowser files={files} showDeleted={showDeleted} refreshFiles={refresh} fileActions={fileActions} openedPath={openedPath} {...props} />;
};
