...
import {Button, Dialog, DialogTitle, DialogActions, DialogContent, FormControl, FormControlLabel, ListItemText, Radio, RadioGroup} from "@mui/material";
...
export const CollectionStatusChangeDialog = ({
    collection,
    setValue,
    onClose,
    classes
    ...
    return <Dialog open={openDialog} data-testid="property-change-dialog" onClose={handleCancel}>
        <DialogTitle id="property-change-dialog-title">Change collection status</DialogTitle>
        <DialogContent>
            <div>
                <FormControl>
                    <RadioGroup aria-label="Available values" name="collection-property-value" className={classes.group} value={selectedValue} onChange={handleValueChange}>
                        {statuses.filter(status => collection.availableStatuses.includes(status)).map(status => <FormControlLabel key={status} value={status} control={<Radio />} label={<ListItemText primary={camelCaseToWords(status, "-")} secondary={descriptionForStatus(status)} />} />)}
                    </RadioGroup>
                </FormControl>
            </div>
        </DialogContent>
        <DialogActions>
            <ConfirmationButton onClick={handleSubmit} disabled={Boolean(!selectedValue)} message={confirmationMessage(selectedValue)} agreeButtonText="Yes" dangerous>
