...
import {COLLECTION_URI, DIRECTORY_URI, FILE_URI, MACHINE_ONLY_URI, SHACL_CLASS, SHACL_DATATYPE, SHACL_DESCRIPTION, SHACL_MAX_COUNT, SHACL_MIN_COUNT, SHACL_NAME, SHACL_PATH} from "../constants";
...
const generateTemplate = vocabulary => {
    const userProps = flatMap([FILE_URI, DIRECTORY_URI, COLLECTION_URI].map(uri => determinePropertyShapesForTypes(vocabulary, [uri]))).filter(ps => !getFirstPredicateValue(ps, MACHINE_ONLY_URI));
    const uniqueProps = [...new Set(userProps.map(ps => getFirstPredicateId(ps, SHACL_PATH))).values()].map(iri => userProps.find(ps => getFirstPredicateId(ps, SHACL_PATH) === iri));
    ...
    const doc = uniqueProps.map(ps => ['# ', getFirstPredicateValue(ps, SHACL_NAME), getFirstPredicateValue(ps, SHACL_DESCRIPTION, ""), typename(ps), cardinality(ps), getFirstPredicateId(ps, SHACL_PATH)]);
    const entityNames = uniqueProps.filter(ps => !getFirstPredicateId(ps, SHACL_DATATYPE)).map(ps => JSON.stringify(getFirstPredicateValue(ps, SHACL_NAME)).replaceAll('"', "'"));
...
const MetadataCard = props => {
    ...
    const uploadMetadata = file => {
        ...
        LocalFileAPI.uploadMetadata(metadataUploadPath, file).then(() => enqueueSnackbar('Metadata have been successfully uploaded')).catch(e => {
            const errorContents = <DialogContentText>
                <Typography style={{
    ...
    return <Card {...rootProps} className={dropzoneClassName()}>
        {inputProps && <input {...inputProps} />}
        <CardHeader titleTypographyProps={{
            variant: 'h6'
        }} title={title} subheader={metadataUploadPath && 'Drag \'n\' drop a metadata file here or click the edit button below to see all available fields.'} avatar={avatar} style={{
            wordBreak: 'break-word'
        }} action={<>
            {metadataUploadPath && (uploadingMetadata ? <CircularProgress size={10} /> : <Tooltip interactive title={<>
                <div>Upload metadata in CSV format.</div>
                <div>
                    {'Download '}
                    <Link download="metadata.csv" href={'data:text/csv;charset=utf-8,' + encodeURIComponent(fileTemplate)}>template file
                    </Link>
                </div>
            </>}>
                <IconButton onClick={open} size="medium"><CloudUpload /></IconButton>
            </Tooltip>)}
            <IconButton onClick={toggleExpand} aria-expanded={expanded} aria-label="Show more" className={expanded ? classes.expandOpen : ''} size="medium">
                <ExpandMore />
            </IconButton>
        </>} />
...
const PathMetadata = React.forwardRef(({
    ...
    const {
        ...
        error,
        loading
    } = useAsync(() => LocalFileAPI.stat(path, showDeleted), [path]);
    let body;
    let isDirectory;
    ...
    return <MetadataCard ref={ref} title={`Metadata for ${relativePath(path)}`} avatar={avatar} forceExpand={forceExpand} metadataUploadPath={hasEditRight && forceExpand && isDirectory && path}>
        {body}
    </MetadataCard>;
});
...
export const CollectionInformationDrawer = (props: CollectionInformationDrawerProps) => {
    ...
    if (!collection) {
        return atLeastSingleCollectionExists && inCollectionsBrowser && <EmptyInformationDrawer message="Select a collection to display its metadata" />;
    }

    const hasEditRight = collection && collection.canWrite;
    return <>
        <CollectionDetails collection={collection} onChangeOwner={props.onChangeOwner} loading={loading} setBusy={props.setBusy} />
        <MetadataCard title={`Metadata for ${collection.name}`} avatar={<Folder />} forceExpand={paths.length === 0} metadataUploadPath={hasEditRight && paths.length === 0 && collection.name}>
            <LinkedDataEntityFormWithLinkedData subject={collection.iri} hasEditRight={hasEditRight} setHasCollectionMetadataUpdates={setHasCollectionMetadataUpdates} />
        </MetadataCard>
        {paths.map((metadataPath, index) => <PathMetadata key={metadataPath} path={metadataPath} showDeleted={showDeleted} hasEditRight={hasEditRight} forceExpand={index === paths.length - 1} />)}
    </>;
};
...
const ContextualCollectionInformationDrawer = ({
    ...
    return <SnackbarProvider maxSnack={3}>
        <CollectionInformationDrawer {...props} loading={loading} collection={collection} showDeleted={showDeleted} atLeastSingleCollectionExists={atLeastSingleCollectionExists} />
    </SnackbarProvider>;
};
