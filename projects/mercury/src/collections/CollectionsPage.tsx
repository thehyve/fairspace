...
const CollectionsPage = (props: CollectionsPageProperties) => {
  ...
  const showMetadataSearchButton = currentUser && currentUser.canViewPublicMetadata && views && views.some(v => v.name === RESOURCES_VIEW);
  return <CollectionBreadcrumbsContextProvider>
            ...
            <Grid container justifyContent="space-between" spacing={1}>
                <Grid item className={classes.topBar}>
                    <Grid container>
                        <Grid item xs={6}>
                            <SearchBar placeholder="Search in all collections" onSearchChange={handleSearch} disabled={collectionsLoading || collectionsError || !collections || collections.length === 0} />
                        </Grid>
                        ...
                        <Grid item xs={2} className={classes.topBarSwitch}>
                            <FormControlLabel control={<Switch color="primary" checked={showDeletedCollections} onChange={() => setShowDeletedCollections(!showDeletedCollections)} />} label="Show deleted" />
                        </Grid>
                    </Grid>
                </Grid>
            </Grid>
            <Grid container spacing={1}>
                <Grid item className={classes.centralPanel}>
                    <CollectionBrowser setBusy={setBusy} isSelected={collection => isSelected(collection.iri)} toggleCollection={collection => toggleCollection(collection.iri)} workspaceIri={workspaceIri} showDeletedCollections={showDeletedCollections} />
                </Grid>
                <Grid item className={classes.sidePanel}>
                    <CollectionInformationDrawer inCollectionsBrowser setBusy={setBusy} selectedCollectionIri={selected} onChangeOwner={() => {
          if (workspaceIri) {
            ...
            {showConfirmDialog ? <ConfirmationDialog open title="Unsaved changes" content={'You have unsaved changes, are you sure you want to navigate away?' + ' Your pending changes will be lost.'} agreeButtonText="Navigate" disagreeButtonText="back to form" onAgree={handleConfirmSwitchCollection} onDisagree={handleCancelSwitchCollection} /> : null}
