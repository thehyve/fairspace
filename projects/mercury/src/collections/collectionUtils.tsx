...
import type {AccessLevel, AccessMode, Collection, CollectionPermissions, Permission, PrincipalPermission, Status} from "./CollectionAPI";
...
const parsePermissions = value => (typeof value !== 'string' ? [] : value.split(',').map(s => s.split(' '))).map(([iri, access]) => ({
    iri,
    access
}));
