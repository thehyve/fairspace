...
const copyProperties = (properties: CollectionProperties): CollectionProperties => ((fields.reduce((copy, field) => {
  copy[field] = properties ? properties[field] || '' : '';
  return copy;
}, {}) as any) as CollectionProperties);

const havePropertiesChanged = (collection: CollectionProperties, properties: CollectionProperties) => !collection || fields.some(field => collection[field] !== properties[field]);
...
type CollectionEditorProps = {
  ...
  addCollection: (arg0: CollectionProperties) => Promise<void>;
  updateCollection: (arg0: Collection) => Promise<void>;
  renameCollection: (arg0: Collection) => Promise<void>;
  onClose: () => void;
...
export class CollectionEditor extends React.Component<CollectionEditorProps, CollectionEditorState> {
  ...
  state = {
    editing: true,
    saveInProgress: false,
    properties: this.props.collection ? copyProperties(this.props.collection) : {
  ...
  handleAddCollection = (properties: CollectionProperties) => {
    ...
    this.onSaveStart();
    return addCollection(properties).then(() => {
      this.onSaveComplete();
      this.close();
    }).catch((err: Error) => {
      this.onSaveComplete();
      ...
      if (err.message.includes('name already exists') || err.message.includes('status code 409')) {
        ErrorDialog.showError('Collection name must be unique', 'Collection name is already in use. Please choose a unique name.');
        return;
      }
  ...
  handleCollectionRename = (target: string) => {
    ...
    return renameCollection(collection.name, target).then(() => {
      this.onSaveComplete();
  ...
  editNameEnabled = !this.props.collection || this.props.collection && this.props.collection.canManage && this.props.collection.accessMode !== 'DataPublished';
  isSaveEnabled = () => !this.state.saveInProgress && isInputValid(this.state.properties) && havePropertiesChanged(this.props.collection, this.state.properties);
  ...
  renderCollectionNameHelperText() {
    return <span>
                ...
                <span className={this.validateNameStartsWithWorkspaceCode() ? this.props.classes.textHelperBasic : this.props.classes.textHelperWarning}>
                    <br />
                    {!this.validateNameStartsWithWorkspaceCode() && <span>
  ...
  render() {
    return <Dialog open={this.state.editing} onClose={this.props.onClose} aria-labelledby="form-dialog-title">
                <DialogTitle id="form-dialog-title">
                    {this.props.collection ? 'Edit collection' : 'Add collection'}
                </DialogTitle>
                <DialogContent>
                    {this.props.collection && <DialogContentText>
                            Be aware that renaming of a large collection can take more time.
                        </DialogContentText>}
                    <TextField autoFocus margin="dense" id="name" label="Name" helperText={this.renderCollectionNameHelperText()} value={this.state.properties.name} name="name" onChange={event => this.handleInputChange('name', event.target.value)} fullWidth required disabled={!this.editNameEnabled} error={!isNameValid(this.state.properties.name)} />
                    <TextField margin="dense" multiline id="description" label="Description" name="description" value={this.state.properties.description} onChange={event => this.handleInputChange('description', event.target.value)} fullWidth />
                </DialogContent>
                <DialogActions>
...
const ContextualCollectionEditor = props => {
  ...
  return <CollectionEditor {...props} history={history} addCollection={addCollection} updateCollection={updateCollection} renameCollection={renameCollection} />;
};
