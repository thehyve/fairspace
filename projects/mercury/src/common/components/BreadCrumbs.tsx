...
const BreadCrumbs = ({
    ...
    return <Breadcrumbs aria-label="Breadcrumbs" className={classes.root}>
        {allSegments.map(({
            label,
            icon,
            href
        }, idx) => <Typography component="span" key={href} className={classes.link} color={idx === allSegments.length - 1 ? 'textPrimary' : 'inherit'}>
            {icon ? <div className={classes.icon}>{icon}</div> : undefined}
            {idx === allSegments.length - 1 ? label : <Link component={RouterLink} className={classes.link} color="inherit" to={href}>
                {label}
            </Link>}
        </Typography>)}
    </Breadcrumbs>;
...
BreadCrumbs.propTypes = {
    additionalSegments: PropTypes.arrayOf(PropTypes.shape({
