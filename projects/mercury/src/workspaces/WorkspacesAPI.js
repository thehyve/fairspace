...
class WorkspacesAPI {
    ...
    createWorkspace(workspace: WorkspaceProperties): Promise<WorkspaceProperties> {
        return axios
            .put(workspacesUrl, JSON.stringify(workspace), {
    ...
    deleteWorkspace(workspaceIri: string): Promise<WorkspaceProperties> {
        return axios
            .delete(`${workspacesUrl}?workspace=${encodeURI(workspaceIri)}`, {
                headers: {Accept: 'application/json'}
            })
    ...
    getWorkspaceRoles(workspaceIri: string): Promise<WorkspaceUserRole[]> {
        return axios
            .get(`${workspacesUrl}users/?workspace=${encodeURI(workspaceIri)}`, {
                headers: {Accept: 'application/json'}
            })
            .then(extractJsonData)
    ...
    setWorkspaceRole(workspace: string, user: string, role: string): Promise<void> {
        return axios
            .patch(`${workspacesUrl}users/`, JSON.stringify({workspace, user, role}), {
