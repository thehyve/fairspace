    ...
    return <>
        <Dialog open={!creating} onClose={onClose} aria-labelledby="form-dialog-title" fullWidth maxWidth="sm">
            ...
            <DialogContent style={{
                ...
                <TextField autoFocus margin="dense" id="code" label="Code" data-testid="Code" helperText={renderCodeHelperText()} value={value} name="code" onChange={event => setValue(event.target.value)} fullWidth required error={!isCodeValid()} />
            </DialogContent>
            <DialogActions>
                <Button type="submit" form="formId" data-testid="submit-button" disabled={value === null || !isCodeValid()} color="primary" variant="contained" onClick={onDialogSubmit}>
                        Save
                </Button>
