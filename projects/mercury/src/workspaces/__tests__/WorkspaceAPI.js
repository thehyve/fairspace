...
describe('WorkspacesAPI', () => {
    it('Fetches workspaces', async () => {
        ...
        expect(workspaces.map((workspace: Workspace) => workspace.code)).toEqual(
            dummyWorkspaces.map(workspace => workspace.code)
        );
    });
    ...
    it('Failure to create a workspace is handled correctly', () => {
        ...
        return expect(workspacesAPI.createWorkspace(workspaceData)).rejects.toThrow(
            'Failure while creating a workspace'
        );
    });
