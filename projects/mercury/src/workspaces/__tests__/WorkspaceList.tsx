...
const wrap = element => <MemoryRouter>
...
describe('WorkspaceList', () => {
    it('shows message when no workspaces are available', () => {
        const {
            getByText
        } = render(wrap(<WorkspaceList workspaces={[]} />));
        expect(getByText(/No workspaces available/i)).toBeInTheDocument();
    });
    it('displays the list of workspaces', () => {
        ...
        const {
            getByText
        } = render(wrap(<WorkspaceList workspaces={workspaces} />));
