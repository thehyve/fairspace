...
class WorkspacesAPI {
    getWorkspaces(): Promise<Workspace[]> {
        ...
        }).then(extractJsonData).catch(handleHttpError("Failure when retrieving a list of workspaces"));
    }
    ...
    createWorkspace(workspace: WorkspaceProperties): Promise<WorkspaceProperties> {
        ...
        }).then(extractJsonData).catch(handleHttpError("Failure while creating a workspace"));
    }
    ...
    getWorkspaceRoles(workspaceIri: string): Promise<WorkspaceUserRole[]> {
        ...
        }).then(extractJsonData).then(roles => Object.entries(roles).map(([iri, role]) => ({
            iri,
            role
    ...
    setWorkspaceRole(workspace: string, user: string, role: string): Promise<void> {
        return axios.patch(`${workspacesUrl}users/`, JSON.stringify({
            workspace,
            user,
            role
        }), {
