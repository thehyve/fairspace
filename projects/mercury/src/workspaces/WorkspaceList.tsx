...
const WorkspaceList = (props: WorkspaceListProps) => {
    ...
    const {
        ...
        orderBy,
        toggleSort
    } = useSorting(workspaces, columns, 'code');
    const {
        page,
        setPage,
        rowsPerPage,
    ...
    if (!props.workspaces || props.workspaces.length === 0) {
        return <MessageDisplay message="No workspaces available" variant="h6" withIcon={false} isError={false} messageColor="textSecondary" />;
    }
        ...
        <Paper style={{
            ...
            <TableContainer>
                <Table>
                    <TableHead>
                        <TableRow>
                            {Object.entries(columns).map(([key, column]) => <TableCell key={key} align={column.align ? column.align : 'inherit'}>
                    ...
                    <TableBody>
                        {pagedItems.map((workspace: Workspace) => <TableRow key={workspace.iri} hover onDoubleClick={() => onWorkspaceDoubleClick(workspace)}>
                            <TableCell style={{
                                maxWidth: 32,
                                width: 32
                            }} scope="row" key="canCollaborate">
                            ...
                            <TableCell style={{
                                ...
                                <ListItemText style={{
                                    margin: 0
                                }} primary={workspace.canCollaborate ? <Link component="button" onClick={e => {
                                    e.stopPropagation();
                                    ...
                                    {workspace.code}
                                </Link> : workspace.code} secondary={workspace.title} />
                            </TableCell>
                            <TableCell align="right" style={{
                                maxWidth: 32,
                                width: 32
                            }} scope="row" key="collectionCount">
                                {workspace.summary ? workspace.summary.nonDeletedCollectionCount : ''}
                            </TableCell>
                            <TableCell align="right" style={{
                                maxWidth: 32,
                                width: 32
                            }} scope="row" key="memberCount">
                                {workspace.summary ? workspace.summary.memberCount : ''}
                            </TableCell>
                            <TableCell style={{
                                maxWidth: 150,
                                width: 150
                            }} scope="row" key="managers">
                                {workspace.managers ? workspace.managers.map(m => <EmailChip key={m.iri} email={m.email} label={m.name} />) : ''}
                            </TableCell>
                            <TableCell style={{
                                maxWidth: 32,
                                width: 32
                ...
                <TablePagination rowsPerPageOptions={[5, 10, 25, 100]} component="div" count={props.workspaces.length} rowsPerPage={rowsPerPage} page={page} onPageChange={(e, p) => setPage(p)} onRowsPerPageChange={e => setRowsPerPage(e.target.value)} style={{
                    overflowX: "hidden"
