...
export const UserPermissionsTable = ({
    ...
    const canManagePermission: boolean = (permission: Permission) => canManage && currentUser && permission.iri !== currentUser.iri;

    const renderAccessLevelDropdown = (selectedPermission: Permission, accessLevelOptions: AccessLevel[]) => <FormControl>
        <Select value={selectedPermission.access} onChange={v => handleChangePermission({...selectedPermission,
            access: v.target.value})} className={classes.accessDropdown}>
            {accessLevelOptions.map(access => <MenuItem key={access} value={access}>
    ...
    return <Table size="small" className={classes.table}>
        <TableBody className={classes.tableBody}>
            {selectedPermissions.map(p => {
                ...
                return <TableRow key={p.iri} className={classes.tableRow}>
                    ...
                    <TableCell className={classes.accessCell}>
                        {canManageCurrentPermission && accessLevelOptions.length > 1 ? renderAccessLevelDropdown(p, accessLevelOptions) : <div>
                    ...
                    <TableCell width={40} className={classes.iconCell} align="right">
                        <IconButton onClick={() => handleDeletePermission(p)} disabled={!canManageCurrentPermission} size="medium">
                            <Close />
                        </IconButton>
                    </TableCell>
                </TableRow>;
