...
export const AlterWorkspacePermissionsDialog = ({
    ...
    const renderSelectedWorkspaceList = () => <div className={classes.accessLevelControl}>
        <Typography component="p">
                Selected workspaces
        </Typography>
        <WorkspacePermissionsTable emptyPermissionsText="No workspace selected." selectedPermissions={selectedPermissions} handleDeleteSelectedPermission={handleDeleteSelectedPermission} canManage={collection.canManage} />
    </div>;

    const renderWorkspaceSelector = () => <PermissionCandidateSelect disableClearable loadOptionsOnMount={false} permissionCandidates={permissionCandidates} onChange={p => handleAddSelectedPermission({...p,
        access: "Read"})} filter={p => p.iri !== collection.ownerWorkspace && !selectedPermissions.some(sp => sp.iri === p.iri)} label="Select workspace" autoFocus />;
