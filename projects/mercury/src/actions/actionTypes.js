// Account
export const USER                   = "USER";
export const AUTHORIZATIONS         = "AUTHORIZATIONS";

// Clipboard
export const CLIPBOARD_CLEAR        = "CLIPBOARD_CLEAR";
export const CLIPBOARD_CUT          = "CLIPBOARD_CUT";
export const CLIPBOARD_COPY         = "CLIPBOARD_COPY";
export const CLIPBOARD_PASTE        = "CLIPBOARD_PASTE";

// Collection browser
export const OPEN_INFODRAWER        = "OPEN_INFODRAWER";
export const CLOSE_INFODRAWER       = "CLOSE_INFODRAWER";
export const OPEN_COLLECTION        = "OPEN_COLLECTION";
export const SELECT_COLLECTION      = "SELECT_COLLECTION";
export const DESELECT_COLLECTION    = "DESELECT_COLLECTION";
export const OPEN_PATH              = "OPEN_PATH";
export const SELECT_PATH            = "SELECT_PATH";
export const DESELECT_PATH          = "DESELECT_PATH";

// Collections
export const COLLECTIONS            = "COLLECTIONS";
export const INVALIDATE_COLLECTIONS = "INVALIDATE_COLLECTIONS";
export const ADD_COLLECTION         = "ADD_COLLECTION";
export const UPDATE_COLLECTION      = "UPDATE_COLLECTION";
export const DELETE_COLLECTION      = "DELETE_COLLECTION";

// Files
export const FILES                  = "FILES";
export const INVALIDATE_FILES       = "INVALIDATE_FILES";
export const RENAME_FILE            = "RENAME_FILE";
export const DELETE_FILE            = "DELETE_FILE";
export const UPLOAD_FILES           = "UPLOAD_FILES";
export const CREATE_DIRECTORY       = "CREATE_DIRECTORY";

// Metadata
export const METADATA               = "METADATA";
export const INVALIDATE_METADATA    = "INVALIDATE_METADATA";
export const UPDATE_METADATA        = "UPDATE_METADATA";
export const METADATA_COMBINATION   = "METADATA_COMBINATION";
export const METADATA_VOCABULARY    = "METADATA_VOCABULARY";
export const METADATA_ENTITIES      = "METADATA_ENTITIES";
export const METADATA_ALL_ENTITIES  = "METADATA_ALL_ENTITIES";
export const METADATA_NEW_ENTITY    = "METADATA_NEW_ENTITY";
export const METADATA_URI_BY_PATH   = "METADATA_URI_BY_PATH";

// Permissions
export const PERMISSIONS = 'PERMISSIONS';
export const ALTER_PERMISSION = 'ALTER_PERMISSION';
export const USERS = 'USERS';

// Workspace
export const WORKSPACE = 'WORKSPACE';

// UI
export const TOGGLE_MENU = 'TOGGLE_MENU';
