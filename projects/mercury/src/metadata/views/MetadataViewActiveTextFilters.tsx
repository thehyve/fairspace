...
export const MetadataViewActiveTextFilters = (props: MetadataViewActiveTextFiltersProperties) => {
  ...
  return <div>
            <Grid container direction="row" justifyContent="flex-start" alignItems="center" spacing={1} className={classes.activeTextFiltersBox}>
                <Grid container item xs>
                    ...
                    {Object.entries(textFiltersObject).filter(([, value]) => value !== null && value !== "").map(([field, value]) => <Grid key={`activeTextFilters_${field}`} item>
                                <Chip key={`chip-${field}`} label={`${columns.find(c => c.name === field).title}: ${value}`} style={{
            marginLeft: 5
          }} onDelete={() => clearSingleTextFilter(field)} />
                            </Grid>)}
                </Grid>
                <Grid key="activeTextFiltersClearButton" item xs={2}>
                    <IconButton onClick={clearTextFilters} title="Clear text filters" className={classes.clearButton} size="medium">
                        <Close color="error" />
                    </IconButton>
                </Grid>
            </Grid>
