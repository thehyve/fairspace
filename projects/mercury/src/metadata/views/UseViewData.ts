...
const useViewData = (view: string, filters: MetadataViewFilter[], textFiltersObject: Record<string, any>, locationContext: string, rowsPerPage: number): ViewData => {
  ...
  const textFilters: MetadataViewFilter[] = Object.entries(textFiltersObject).filter(([field, value]) => field !== null && value !== "").map(([field, value]) => ({
    field,
    prefix: value
  }));
  const allFilters = !locationContext ? [...filters, ...textFilters] : [...filters.filter(f => ![LOCATION_FILTER_FIELD].includes(f.field)), locationFilter, ...textFilters];
  ...
  const refreshAll = useCallback(() => {
    ...
    fetchViewData(0, rowsPerPage).then((d: MetadataViewData) => {
      setData(d);
