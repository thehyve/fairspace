...
type MetadataViewTableContainerProperties = {
  ...
  textFiltersObject: Record<string, any>;
...
export const MetadataViewTableContainer = (props: MetadataViewTableContainerProperties) => {
    ...
    const [visibleColumnNames, setVisibleColumnNames] = useStateWithLocalStorage(`${SESSION_STORAGE_VISIBLE_COLUMNS_KEY_PREFIX}_${view.toUpperCase()}`, columns.map(c => c.name));
    ...
    const {
        ...
        loadingCount,
        refreshDataOnly
    } = useViewData(view, filters, textFiltersObject, locationContext, rowsPerPage);
    ...
    const getCsvValuesForAttribute = (row, attribute) => {
        ...
        return Object.values(row[attribute]).map(value => ((value && value.label) ?? "-").replaceAll(";", ".,")).join(",");
    };
    ...
    const saveTableExtraStorage = () => {
        ...
        ExtraLocalStorage.deleteAllInDirectory(ANALYSIS_EXPORT_SUBPATH).then(() => ExtraLocalStorage.upload(csvFile, fileName, ANALYSIS_EXPORT_SUBPATH, true)).then(() => setCurrentSelectionExported(true)).catch((err: Error) => ErrorDialog.showError('Could not export the selection to analysis', err.message)).finally(() => setExportToAnalysisLoading(false));
    };

    const renderMessages = () => <div className={classes.messageBox}>
        {count.timeout && <MessageDisplay small message="The count request timed out." />}
        {hasInactiveFilters && <MessageDisplay color="primary" isError={false} small message="Apply filters to see data matching your current selection." />}
    </div>;

    const renderColumnSelector = () => <Popover open={columnSelectorOpen} onClose={handleColumnSelectorClose} anchorEl={anchorEl} anchorOrigin={{
        vertical: 'center',
        horizontal: 'left'
    }} transformOrigin={{
        vertical: 'top',
        ...
        <FormControl className={classes.viewColumnsFormControl}>
            ...
            <FormGroup>
                {columns.map(column => <FormControlLabel key={column.name} control={<Checkbox checked={visibleColumnNames.includes(column.name)} disabled={column.name === idColumn.name} onChange={handleVisibleColumnsChange} name={column.name} icon={<CheckBoxOutlineBlankIcon fontSize="small" />} checkedIcon={<CheckBoxIcon fontSize="small" />} />} label={column.title} />)}
            </FormGroup>
        </FormControl>
    </Popover>;

    const renderTableSettings = () => <div className={classes.tableSettings}>
        <IconButton aria-label="Show/hide columns" title="Show/hide columns" onClick={handleColumnSelectorButtonClick} size="medium">
    ...
    const labelDisplayedRows = ({
        ...
        <Typography variant="body2" component="span" display="inline" style={{
            fontWeight: "bold"
        }}>
            {totalCount !== undefined && totalCount !== -1 ? totalCount.toLocaleString() : "more than " + to}
            {countIsLoading && <CircularProgress size={14} style={{
                marginLeft: 3
            }} />}
        </Typography>
    </span>;
    ...
    const renderMetadataViewTable = () => {
        ...
        return <MetadataViewTable {...props} visibleColumnNames={visibleColumnNames} idColumn={idColumn} data={data} loading={!data || loading} history={history} textFiltersObject={textFiltersObject} setTextFiltersObject={setTextFiltersObject} checkboxes={rowCheckboxes} setCheckboxState={setRowCheckboxState} />;
    };
    ...
    return <Paper>
        ...
        <LoadingOverlayWrapper loading={!data || loading}>
            <MetadataViewActiveTextFilters textFiltersObject={textFiltersObject} setTextFiltersObject={setTextFiltersObject} columns={columns} />
            ...
            <div className={classes.footerButtonDiv}>
                ...
                <Tooltip title="Download as csv" className={classes.exportButton}>
                    <span>
                        <IconButton aria-label="Download as csv" color="primary" disabled={checkedCount === 0} onClick={exportTable}>
                            <GetAppIcon fontSize="small" />
                        </IconButton>
                    </span>
                </Tooltip>
                {exportToAnalysisEnabled && <ProgressButton active={exportToAnalysisLoading}>
                    <Tooltip title="Export to Jupiter Analysis" className={classes.exportButton}>
                        <span>
                            <IconButton aria-label="Export to Jupiter Analysis" color="primary" disabled={checkedCount === 0 || currentSelectionExported} onClick={saveTableExtraStorage}>
                                {currentSelectionExported ? <Check fontSize="small" /> : <Addchart fontSize="small" />}
                            </IconButton>
                        </span>
                    </Tooltip>
                </ProgressButton>}
                <TablePagination rowsPerPageOptions={[5, 10, 25, 100]} component="div" count={count && isNonEmptyValue(count.count) ? count.count : -1} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} className={classes.tableFooter} labelDisplayedRows={d => labelDisplayedRows({...d,
                    countIsLoading: loadingCount})} ActionsComponent={TablePaginationActions} />
            </div>
