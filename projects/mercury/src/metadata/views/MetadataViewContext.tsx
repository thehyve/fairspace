...
export const MetadataViewProvider = ({
  ...
  const {
    ...
    loading,
    refresh
  } = useAsync(() => metadataViewApi.getViews(), []);
  const [filters, setFilters] = useStateWithSessionStorage(SESSION_STORAGE_METADATA_FILTERS_KEY, []);
  ...
  const updateFilters = (filterCandidates: MetadataViewFilter[]) => {
    setFilters([...filters.filter(f => !filterCandidates.some(u => u.field === f.field)), ...filterCandidates.filter(f => f.values && f.values.length > 0 || isNonEmptyValue(f.min) || isNonEmptyValue(f.max) || f.prefix != null || f.booleanValue != null)]);
  };

  return <MetadataViewContext.Provider value={{
    views: data.views,
    filters,
    updateFilters,
    clearAllFilters,
