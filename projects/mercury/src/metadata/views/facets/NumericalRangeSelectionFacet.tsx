...
const NumericalRangeSelectionFacet = (props: MetadataViewFacetProperties) => {
    ...
    const renderInput = (inputValue, onInputChange, placeholder) => <Input value={nonEmptyNumber(inputValue, "")} margin="dense" onChange={onInputChange} onBlur={handleBlur} error={!!validationError} inputProps={{
        'step': 1,
        'min': minValue,
        'max': maxValue,
        'type': 'number',
    ...
    const renderSlider = () => <Slider value={getSliderValue()} track={dynamicSliderValue[1] === null && isNonEmptyValue(dynamicSliderValue[0]) ? 'inverted' : 'normal'} onChange={handleSliderChange} onChangeCommitted={() => commitChange(dynamicSliderValue)} valueLabelDisplay="auto" aria-labelledby="range-slider" getAriaValueText={() => dynamicSliderValue} min={minValue} max={maxValue} />;

    return <div>
        {minValue !== null && maxValue !== null ? <Grid container spacing={2} alignItems="center" className={classes.numericalContent}>
            <Grid item xs={3}>
        ...
        </Grid> : <Grid container spacing={3} alignItems="center" className={classes.numericalContent}>
            <Grid item xs={6}>
