...
export const MetadataView = (props: MetadataViewProperties) => {
    ...
    const toggleRow = useCallback((entity: MetadataViewEntity) => {
        ...
        toggle(entity); // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
    ...
    const changeTab = useCallback((event, tabIndex) => {
        ...
        handleViewChangeRedirect(views[tabIndex].name); // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [views]);
    const clearFilterCandidates = useCallback(() => {
        setFilterCandidates([]); // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [filterCandidates]);
    const applyFilters = useCallback(() => {
        updateFilters(filterCandidates);
        clearFilterCandidates(); // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const updateFilterCandidates = useCallback((facet: MetadataViewFacet, newValues: any[]) => {
        if (filterCandidates.find(f => f.field === facet.name)) {
            ...
            if (!newValues || existingFilter && existingFilter.values && _.isEqual(existingFilter.values.sort(), newValues.sort()) && (newValues.filter(v => v !== null).length === 0 || existingFilter.value)) {
    ...
    const handleClearFilter = useCallback((facetName: string) => {
        ...
        clearFilter(facetName); // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [filterCandidates]);
    const collectionsFacet = useMemo(() => !locationContext && collections && {
        name: 'location',
        title: "Collection",
        type: 'Term',
    ...
    const facetsEx = useMemo(() => collectionsFacet ? [...facets, collectionsFacet] : facets, [collectionsFacet, facets]);
    ...
    const getPathSegments = () => {
        ...
        segments.reverse().forEach(segment => {
            result.push({
                label: segment,
                href: pathPrefix + encodeURIComponent(path)
            });
    ...
    const areFacetFiltersNonEmpty = useMemo(() => filters && filters.some(filter => facetsEx.some(facet => facet.name === filter.field)), [filters, facetsEx]);
    const areTextFiltersNonEmpty = useMemo(() => textFiltersObject && Object.keys(textFiltersObject).length > 0, [textFiltersObject]);
    ...
    return <BreadcrumbsContext.Provider value={{
        segments: [{
            label: "Metadata",
            href: getMetadataViewsPath(currentView.name),
            icon: <Assignment />
        }]
    }}>
        <BreadCrumbs additionalSegments={getPathSegments(locationContext)} />
        {(areFacetFiltersNonEmpty || areTextFiltersNonEmpty) && <Grid container justifyContent="space-between" direction="row-reverse">
            ...
            {areFacetFiltersNonEmpty && <Grid item container xs alignItems="center" spacing={1} className={classes.activeFilters}>
                <Grid item>
            ...
            <Grid item className={`${classes.overallPanel} ${isClosedPanel && classes.overallPanelFullWidth}`}>
                <Grid container direction="row" spacing={1} wrap="nowrap">
                    <Grid item className={classes.leftPanel}>
                        <MetadataViewFacets views={views} filters={filters} facetsEx={facetsEx} clearFilterCandidates={clearFilterCandidates} filterCandidates={filterCandidates} updateFilterCandidates={updateFilterCandidates} handleClearFilter={handleClearFilter} applyFilters={applyFilters} />
                    </Grid>
                    <Grid item className={classes.centralPanel}>
                        <MetadataViewTabs currentViewIndex={currentViewIndex} idColumn={currentViewIdColumn} changeTab={changeTab} views={views} filters={filters} locationContext={locationContext} selected={selected} toggleRow={toggleRow} hasInactiveFilters={filterCandidates.length > 0} collections={collections} textFiltersObject={textFiltersObject} setTextFiltersObject={setTextFiltersObject} />
                    </Grid>
                </Grid>
            </Grid>
            <Grid item className={classes.rightPanel} hidden={isClosedPanel}>
                <MetadataViewInformationDrawer handleCloseCard={() => setIsClosedPanel(true)} entity={selected} viewIcon=<Assignment /> textFilterLink={getPrefilteringRedirectionLink(selected)} />
            </Grid>
        </Grid>
...
export const ContextualMetadataView = (props: ContextualMetadataViewProperties) => {
    ...
    return <MetadataView {...props} facets={facets} views={views} locationContext={currentViewName === RESOURCES_VIEW && locationContext} currentViewName={currentViewName} filters={filters} handleViewChangeRedirect={handleViewChangeRedirect} />;
};
