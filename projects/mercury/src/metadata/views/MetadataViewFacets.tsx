...
export const MetadataViewFacets = (props: MetadataViewFacetsProperties) => {
    ...
    const renderSingleFacet = (facet: MetadataViewFacet) => {
        ...
        return facetOptions && facetOptions.length > 0 && <Grid key={facet.name} item>
            <Facet type={facet.type} title={facet.title} options={facetOptions} onChange={values => updateFilterCandidates(facet, values)} extraClasses={classes.facet} activeFilterValues={activeFilterValues} clearFilter={() => handleClearFilter(facet.name)} />
        </Grid>;
    };
    ...
    const renderFacets = (view: MetadataViewOptions) => {
        ...
        return viewFacets.length > 0 && <Grid key={view.name} className={classes.facetGroup} container item direction="column" justifyContent="flex-start">
            ...
            {// location is the collection location, which we will group under resources
                view.name.toLowerCase() === 'resource' ? facetsEx.filter(facet => facet.name.toLowerCase().startsWith('location')).map(facet => renderSingleFacet(facet, filters, filterCandidates, updateFilterCandidates, handleClearFilter)) : ""}
        </Grid>;
    };

    const renderFacetConfirmButtons = () => <Grid container spacing={0.5} className={`${classes.confirmFiltersButtonBlock}`}>
        <Grid item xs={4}>
            <Button onClick={clearFilterCandidates} variant="contained" className={classes.confirmFiltersButton} disabled={filterCandidates.length === 0}>
                    Cancel
            </Button>
        </Grid>
        <Grid item xs={8}>
            <Button onClick={applyFilters} variant="contained" color="secondary" className={classes.confirmFiltersButton} disabled={filterCandidates.length === 0}>
                    Apply filters
            </Button>
        </Grid>
    </Grid>;

    return <Grid container item direction="column" justifyContent="flex-start" spacing={1}>
