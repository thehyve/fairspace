...
export const MetadataViewActiveFacetFilters = (props: MetadataViewActiveFacetFiltersProperties) => {
  ...
  const renderActiveFilterValues = (facet, filter) => {
    if (ofRangeValueType(facet.type)) {
      ...
      return <Chip className={facet.backgroundColor} key={`chip-${facet.name}`} label={label} style={{
        marginLeft: 5
      }} onDelete={() => {
    ...
    if (ofBooleanValueType(facet.type)) {
      return <Chip className={facet.backgroundColor} key={`chip-${facet.name}`} label={filter.booleanValue} style={{
        marginLeft: 5
      }} onDelete={() => {
        const index = filters.indexOf(filter);
        delete filters[index];
    ...
    return filter.values.map(valueIri => {
      ...
      return value && <Chip className={facet.backgroundColor} key={value.value} label={value.label} style={{
        marginLeft: 5
      }} onDelete={() => {
        const index = filters.indexOf(filter);
  ...
  return <Grid container item direction="row" justifyContent="flex-start" alignItems="stretch" spacing={1}>
            {filters && filters.map(filter => {
      if (!isNonEmptyValue(filter.min) && !isNonEmptyValue(filter.max) && (!filter.values || filter.values.length === 0) && filter.booleanValue === null) {
        return null;
      }
      ...
      if (facet) {
        return <Grid key={filter.field} item>
