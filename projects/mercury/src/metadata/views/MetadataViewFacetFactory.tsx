...
const Facet = (props: MetadataViewFacetProperties) => {
    ...
    const cardHeaderAction = <IconButton className={classnames(classes.expand, {
        [classes.expandOpen]: expanded
    }, classes.headerIcon)} onClick={toggleExpand} aria-expanded={expanded} aria-label="Show more" size="medium">
        <ExpandMore />
    </IconButton>;
    const clearFiltersAction = activeFilterValues.length > 0 && <IconButton onClick={clearFilter} aria-label="Clear" className={classes.headerIcon} size="medium">
        <Clear fontSize="small" color="primary" />
    </IconButton>;
    return <Card className={`${classes.root} ${extraClasses}`} variant="outlined">
        <CardHeader className={classes.title} titleTypographyProps={{
            color: "textSecondary",
            variant: "body1"
        }} title={title} avatar={cardHeaderAction} action={clearFiltersAction} />
