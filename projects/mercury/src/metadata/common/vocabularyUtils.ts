 ...
 */
export const getClassesInCatalog = vocabulary => vocabulary.filter(entry => getFirstPredicateId(entry, constants.SHACL_TARGET_CLASS) || entry['@type'] && entry['@type'].includes(constants.RDFS_CLASS) && entry['@type'].includes(constants.SHACL_NODE_SHAPE)).filter(entry => !getFirstPredicateValue(entry, constants.MACHINE_ONLY_URI)).filter(entry => !getFirstPredicateValue(entry, constants.DATE_DELETED_URI));
 ...
 */
export const getNamespaces = (vocabulary, namespaceFilter = () => true) => vocabulary.filter(entry => entry['@type'] && entry['@type'].includes(constants.SHACL_PREFIX_DECLARATION)).filter(namespaceFilter).map(namespace => ({
  id: namespace['@id'],
 ...
 * @param propertyUri
 */
export const determineShapeForProperty = (vocabulary, propertyUri) => vocabulary.find(entry => getFirstPredicateId(entry, constants.SHACL_PATH) === propertyUri && getFirstPredicateValue(entry, constants.SHACL_NAME));
 ...
 * @param type
 */
export const determineImportantPropertyShapes = (vocabulary, type) => determinePropertyShapesForTypes(vocabulary, [type]).filter(shape => getFirstPredicateValue(shape, constants.IMPORTANT_PROPERTY_URI, false));
