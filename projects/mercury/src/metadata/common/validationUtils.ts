...
export const maxLengthValidation = (maxLength, values) => values && values.some(v => v.length > maxLength) ? `Please provide no more than ${maxLength} characters` : null;
...
export const maxCountValidation = (maxCount, values) => values && values.length > maxCount ? `Please provide no more than ${maxCount} values` : null;
export const validateValuesAgainstShape = ({
    shape,
    datatype,
    values,
