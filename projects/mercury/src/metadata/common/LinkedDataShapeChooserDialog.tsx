...
const LinkedDataShapeChooserDialog = props => {
  ...
  return <Dialog open={props.open} onClose={closeDialog} aria-labelledby="form-dialog-title">
            <DialogTitle id="form-dialog-title">Create new entity of type</DialogTitle>
            <DialogContent>
                {props.shapes && props.shapes.length ? <List>
                                {props.shapes.sort(compareBy(getLabel)).map(t => <ListItem key={t['@id']} button onClick={() => props.onChooseShape(t)}>
                                            <ListItemText primary={getLabel(t)} secondary={getFirstPredicateValue(t, consts.SHACL_DESCRIPTION)} />
                                        </ListItem>)}
                            </List> : <LoadingInlay />}
            </DialogContent>
            <DialogActions>
