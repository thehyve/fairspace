...
const AddValueToList = (props: AddValueToListProps) => {
  ...
  return <Grid container spacing={1} alignItems="center" className={classes.addValue} data-testid={"" + labelId}>
            <Grid item xs={isStringValue ? 10 : 12}>
                <AddComponent data-testid="add-value-input" variant="outlined" // label='New value'
      key={serialNumber} property={property} currentValues={values} placeholder="" aria-labelledby={labelId} onChange={val => {
        if (val) {
          if (AddComponent === StringValue) {
            ...
            {isStringValue && <Grid item>
                    <IconButton data-testid="add-btn" title="Add" onClick={() => {
...
export const LinkedDataValuesList = (props: LinkedDataValuesListProps) => {
  ...
  const renderValue = (entry, index) => rowDecorator(entry, <Grid container spacing={1} alignItems="center" onMouseEnter={() => setHoveredIndex(index)} onFocus={() => setHoveredIndex(index)} onBlur={() => setHoveredIndex(null)} onMouseLeave={() => setHoveredIndex(null)} onDoubleClick={() => onOpen(entry)} // eslint-disable-next-line react/no-array-index-key
  key={index}>
        ...
        width: "20rem"
                    ...
                    {columnDefinition.id === LABEL_URI ? <Typography variant="h6">{columnDefinition.getValue(entry, index)}</Typography> : <Typography noWrap>{columnDefinition.getValue(entry, index)}</Typography>}
                </div>
                {showRowDividers && <Divider />}
            </Grid>
            {property.isEditable && isDeleteButtonEnabled(entry) && <Grid item xs={2}>
                        <IconButton data-testid="delete-btn" title="Delete" onClick={() => {
        onDelete(index);
  ...
  const renderValues = () => {
    if (values.length > 20) {
      return <List height={300} itemCount={values.length} itemSize={30}>
                    {renderValueWindowed}
                </List>;
    }
  ...
  return <>
            {showHeader ? <Grid container spacing={1} alignItems="center">
                    <Grid item xs={property.isEditable ? 10 : 12}>
                        {columnDefinition.label}
                    </Grid>
                </Grid> : undefined}
            {renderValues()}
            {isAddButtonEnabled && (property.maxValuesCount === 1 ? <Grid container spacing={1} alignItems="center">
                            <Grid item xs={10}>
                                <AddComponent key={serialNumber} property={property} currentValues={values} placeholder="" aria-labelledby={labelId} onChange={val => {
          if (val) {
            onAdd(val);
            incrementSerialNumber();
          }
                        ...
                        </Grid> : <AddValueToList serialNumber={serialNumber} classes={classes} property={property} values={values} labelId={labelId} addComponent={AddComponent} onAdd={val => {
      if (val) {
        onAdd(val);
        incrementSerialNumber();
      }
