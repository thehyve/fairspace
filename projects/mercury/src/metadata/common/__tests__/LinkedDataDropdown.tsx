...
describe('LinkedDataDropdown', () => {
  it('calls fetchItems with the given types', () => {
    ...
    const wrapper = shallow(<LinkedDataDropdown property={{}} debounce={0} type="http://workspace.ci.fairway.app/vocabulary/PersonConsent" fetchItems={mockFetchItems} />);
    const dropdown = wrapper.find(Dropdown);
    return dropdown.prop("loadOptions")().then(() => {
      expect(mockFetchItems).toHaveBeenCalledTimes(1);
      expect(mockFetchItems).toHaveBeenCalledWith(expect.objectContaining({
