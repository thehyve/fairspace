...
describe('LinkedDataProperty elements', () => {
    ...
    describe('canEdit', () => {
        ...
        it('should not allow adding new entities if property is not editable', () => verifyCanEdit({...defaultProperty,
            isEditable: false}, false));
        it('should not allow adding new entities if property is machineOnly', () => verifyCanEdit({...defaultProperty,
            machineOnly: true}, false));
    });
    describe('inputComponents', () => {
        ...
        const renderTable = property => {
            const wrapper = mount(<ThemeProvider theme={theme}><LinkedDataContext.Provider value={{
                valueComponentFactory
            }}><LinkedDataProperty property={property} /></LinkedDataContext.Provider></ThemeProvider>);
        ...
        it('should use the factory in the context to determine the Add component', () => {
            expect(renderTable({...defaultProperty,
                isRelationShape: false}).prop("addComponent")).toEqual(NumberValue);
        });
        it('should use the factory in the context to determine the edit component', () => {
            expect(renderTable({...defaultProperty,
                isRelationShape: false}).prop("editComponent")).toEqual(ReferringValue);
        });
        it('should render a read-only component for non editable shapes', () => {
            expect(renderTable({...defaultProperty,
                isRelationShape: false,
                isEditable: false}).prop("editComponent")).toEqual(SwitchValue);
        });
        it('should render a read-only component for machine only shapes', () => {
            expect(renderTable({...defaultProperty,
                isRelationShape: false,
                machineOnly: true}).prop("editComponent")).toEqual(SwitchValue);
        });
        it('should render a read-only component for generic IRI resources', () => {
            expect(renderTable({...defaultProperty,
                isRelationShape: false,
                isGenericIriResource: true}).prop("editComponent")).toEqual(SwitchValue);
        });
        it('should render a read-only component for controlled shapes', () => {
            expect(renderTable({...defaultProperty,
                isRelationShape: false,
                allowedValues: ['a']}).prop("editComponent")).toEqual(SwitchValue);
        });
    });
});