...
describe('useLinkedData', () => {
  ...
  it('should fetch linked data when first loaded', () => {
    ...
    render(<LinkedDataParentMock iri={COLLECTION_URI} context={context} />);
    expect(context.fetchLinkedDataForSubject).toHaveBeenCalledTimes(1);
  });
  it('should handle missing linkedData', async () => {
    render(<LinkedDataParentMock iri={"my-subject"} context={defaultContext} />);
  ...
  describe('loading state', () => {
    ...
    it('should be loading if shapes are loading', async () => {
      ...
      render(<LinkedDataParentMock iri={"my-subject"} context={context} />);
  ...
  describe('error state', () => {
    ...
    it.skip('should show some message for no metadata', async () => {
      ...
      await render(<LinkedDataParentMock iri={"my-subject"} context={context} />);
  ...
  it.skip('should return properties based on the type of the entity', async () => {
    const context = {
      ...
      fetchLinkedDataForSubject: () => Promise.resolve([{ ...defaultJsonLd[0],
        '@type': ['http://specific-type']
      }]),
      result: {}
    };
    render(<LinkedDataParentMock iri={'http://subject'} context={context} />);
  ...
  it.skip('should return type info from linked data', async () => {
    ...
    const {
      result,
      waitForNextUpdate
    } = renderHook(() => useLinkedDataNoContext('http://subject', context));
    await waitForNextUpdate();
