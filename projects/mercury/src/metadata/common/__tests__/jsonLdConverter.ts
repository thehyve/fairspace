...
describe('jsonLdConverter', () => {
  describe('fromJsonLd', () => {
    ...
    it('should return values for each predicate', () => {
      ...
      expect(valuesByPredicate[constants.COMMENT_URI].map(v => v.value)).toEqual(['My first collection', 'Some more info']);
    });
    describe('sorting', () => {
    ...
    it('should parse rdf lists correctly', () => {
      ...
      const valuesByPredicate = fromJsonLd(metadata, propertyShapes);
      expect(valuesByPredicate['http://list'].map(v => v.value)).toEqual(['My first collection', 'Some more info']);
    });
