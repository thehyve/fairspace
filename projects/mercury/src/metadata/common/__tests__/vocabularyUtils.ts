// @ts-nocheck
import {contains, determinePropertyShapesForTypes, determineShapeForProperty, getChildSubclasses, getClassesInCatalog, getDescendants, getLabelForType, getMaxCount, getNamespaces, getProperties, isGenericIriResource, isRdfList, isRelationShape, typeShapeWithProperties} from "../vocabularyUtils";
...
describe('typeShapeWithProperties', () => {
    it('collects shape information with properties for a type', () => {
        ...
        expect(researchProjectShape[SHACL_PROPERTY].map(property => property[SHACL_NAME][0]['@value'])).toEqual(['Label', 'Project type', 'Grant type']);
    });
});
describe('determineShapeForProperty', () => {
    it('returns the correct shape given a url', () => {
        expect(determineShapeForProperty(vocabularyJsonLd, 'http://www.w3.org/2000/01/rdf-schema#comment')[SHACL_PATH][0]['@id']).toEqual('http://www.w3.org/2000/01/rdf-schema#comment');
    });
    it('returns the correct shape if there is also a blank node pointing to it', () => {
        expect(determineShapeForProperty(vocabularyJsonLd, 'https://fairspace.nl/ontology#list')[SHACL_PATH][0]['@id']).toEqual('https://fairspace.nl/ontology#list');
    });
...
describe('isRdfList', () => {
    const rdfListShape = {
        [constants.SHACL_NODE]: [{
            '@id': constants.DASH_LIST_SHAPE
        }]
    };
    const nonRdfListShape = {
        [constants.SHACL_DATATYPE]: [{
            '@id': constants.STRING_URI
        }]
    };
...
describe('isGenericResourceIri', () => {
    const genericResourceShape = {
        [constants.SHACL_NODEKIND]: [{
            '@id': constants.SHACL_IRI
        }]
    };
...
describe('isRelationShape', () => {
    it('should return true for relation shapes', () => {
        expect(isRelationShape({
            [constants.SHACL_CLASS]: [{
                '@value': 'SomeClass'
            }]
        })).toBe(true);
    });
    it('should return false for other types of shapes', () => {
        expect(isRelationShape({
            [constants.SHACL_DATATYPE]: ['http://ex.com/type']
        })).toBe(false);
...
describe('getProperties', () => {
    it('returns all properties specified in the vocabulary', () => {
        ...
        expect(result.map(entry => entry.key)).toEqual(expect.arrayContaining(propertyShapes.map(propertyShape => getFirstPredicateId(propertyShape, constants.SHACL_PATH))));
    });
    ...
    it('should use dash:singleLine to determine multiLine status', () => {
        const propertyShapes = [{
            [constants.SHACL_PATH]: [{
                '@id': constants.LABEL_URI
            }],
            [constants.SHACL_DATATYPE]: [{
                '@id': constants.STRING_URI
            }],
            [constants.DASH_SINGLE_LINE]: [{
                '@value': true
            }]
        }, {
            [constants.SHACL_PATH]: [{
                '@id': constants.COMMENT_URI
            }],
            [constants.SHACL_DATATYPE]: [{
                '@id': constants.STRING_URI
            }],
