...
import { generateUuid, getLabel, getLocalPart, getNamespacedIri, getTypeInfo, hasValue, linkLabel, partitionErrors, propertiesToShow, relativeLink, shouldPropertyBeHidden, url2iri, valuesContainsValueOrId } from "../metadataUtils";
...
describe('Metadata Utils', () => {
  ...
  describe('getLabel', () => {
    ...
    it('should keep external urls intact if shortenExternalUris is set to false', () => {
      expect(getLabel({
        '@id': 'http://test.nl/name#lastname'
      }, false)).toEqual('http://test.nl/name#lastname');
    });
    it('should return part of the url after the pound sign', () => {
      expect(getLabel({
        '@id': 'http://test.nl/name#lastname'
      }, true)).toEqual('lastname');
    });
    it('should return part of the url after the last slash if no pound sign present', () => {
      expect(getLabel({
        '@id': 'http://test.nl/name'
      }, true)).toEqual('name');
    });
  });
  describe('relativeLink', () => {
    it('should strip the base URL', () => {
      expect(relativeLink('http://example.com:1234/some/path?query=value#bookmark')).toEqual('/some/path?query=value#bookmark');
    });
    it('should also handle simple URLs', () => {
      expect(relativeLink('http://example.com')).toEqual('example.com');
    });
  });
  ...
  describe('propertiesToShow', () => {
    it('should hide the type of an entity', () => {
      const properties = [{
        key: "@type",
        values: [{
          id: "https://fairspace.nl/ontology#Collection",
          comment: "A specific collection in Fairspace."
        }]
      }, {
  ...
  describe('getTypeInfo', () => {
    const vocabulary = [{
      [constants.SHACL_TARGET_CLASS]: [{
        "@id": "http://example.com/123"
      }],
      [constants.SHACL_NAME]: [{
        '@value': 'Name'
      }],
      [constants.SHACL_DESCRIPTION]: [{
        '@value': 'Description'
      }]
    }];
  ...
  describe('partitionErrors', () => {
    it('returns 2 arrays one for errors of the given subjects other is the rest of errors', () => {
      ...
      expect(partitionErrors(allErrors, 'subject1')).toEqual({
  ...
  describe('hasValue', () => {
    ...
    it('should return false if only an empty string is is present', () => expect(hasValue([{
      value: ""
    }])).toBe(false));
    it('should return true if an id is present', () => expect(hasValue([{
      id: "http://a"
    }])).toBe(true));
    it('should return true if a non-empty value is present', () => expect(hasValue([{
      value: "label"
    }])).toBe(true));
  });
