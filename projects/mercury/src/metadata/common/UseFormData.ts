...
const populateDefaultFormValues = (initialProperties, values, setFormValues) => {
  ...
  initialProperties.forEach(p => {
    ...
    const newValues = !values[p.key] && p.maxValuesCount === 1 && DEFAULTABLE_DATATYPES.includes(p.datatype) ? defaultValue : values[p.key];
...
const useFormData = (values, initialProperties = []) => {
  ...
  const valuesWithUpdates = { ...initialFormValues,
    ...updates
  };
  ...
  const save = (property, newValue: any[]) => {
    const equalToInitialValue = !initialFormValues[property.key] && newValue.length === 0 || _.isEqual(newValue, initialFormValues[property.key]);
