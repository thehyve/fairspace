...
const LinkedDataProperty = ({
  ...
  const labelTooltip = <><Iri iri={path} /><div style={{
      marginTop: 4
    }}>{description}</div></>;
  return <FormControl required={formEditable && minValuesCount > 0} error={formEditable && hasErrors} component="fieldset" style={{
    width: '100%',
    margin: 4
  }}>
            {canEdit || key !== LABEL_URI ? <GenericTooltip interactive leaveDelay={100} title={labelTooltip} enterDelay={TOOLTIP_ENTER_DELAY}>
                            <FormLabel component="legend">
                                {label} {values.length > 1 && "(" + values.length + ")"}
                            </FormLabel>
                        </GenericTooltip> : null}
            <FormGroup>
                {property.isRelationShape ? <LinkedDataRelationTable property={property} values={values} canEdit={canEdit} onAdd={onAdd} onDelete={onDelete} addComponent={addInputComponent} editorPath={editorPath} /> : <LinkedDataInputFieldsTable property={property} values={values} validationErrors={validationErrors} canEdit={canEdit} onAdd={onAdd} onChange={onChange} onDelete={onDelete} labelId={labelId} editComponent={editInputComponent} addComponent={addInputComponent} />}
            </FormGroup>
