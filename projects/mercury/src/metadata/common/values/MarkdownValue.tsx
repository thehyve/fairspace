...
const MarkdownValue = props => {
  ...
  return <div onClick={() => setShowEdit(true)}>
            {showEdit || !props.entry.value || !props.entry.value.trim() ? <BaseInputValue {...props} autoFocus={showEdit && !!props.entry.value} onBlur={() => setShowEdit(false)} type="text" /> : <ReactMarkdown>{props.entry.value}</ReactMarkdown>}
        </div>;
};
