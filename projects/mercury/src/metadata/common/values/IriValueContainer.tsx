...
export const IriValue = React.forwardRef(({
  ...
  const namespaceOptions = [noNamespace, ...namespaces.map(n => ({
    id: n.id,
    label: n.label,
    value: n.namespace,
  ...
  return <Grid container alignItems="flex-end" justifyContent="space-between" spacing={1}>
            <Grid item xs={4}>
                <Autocomplete options={namespaceOptions} value={namespace || defaultNamespace} onChange={(e, v) => {
        onNamespaceChange(v);
      }} getOptionDisabled={option => option.disabled} getOptionLabel={option => option.label} renderInput={props => <TextField ref={ref} fullWidth {...props} />} />
            </Grid>
            <Grid item xs={8} style={{
      paddingTop: 8,
      paddingBottom: 0
    }}>
                <BaseInputValue property={{}} entry={{
        value: localPart
      }} onChange={e => onLocalPartChange(e.value)} type="url" />
            </Grid>
        </Grid>;
});
IriValue.propTypes = {
