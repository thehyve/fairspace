...
const InputWithAddition = ({
  ...
  const renderAddFunctionality = () => {
    ...
    return <>
                ...
                {adding && <NewLinkedDataEntityDialog shape={shape} onCreate={onCreate} onClose={handleCloseDialog} requireIdentifier={requireIdentifier} />}
            </>;
  };

  return <Grid container justifyContent="space-between" spacing={1}>
            <Grid item xs={10}>
                {children}
            </Grid>
            <Grid item xs={2}>
