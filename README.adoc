...
For secure authentication, it is strongly advised to use the _OpenID Connect (OICD) / OAuth2_ workflow.
...
.Code to obtain the authorisation header (Python)
[%collapsible]
====
[source, python]
----
...
def auth():
    ...
    token = fetch_access_token(keycloak_url, realm, client_id, client_secret, username, password)
    return f'Bearer: {token}'
----
====

.Code to obtain the authorisation header (bash, curl)
[%collapsible]
====
...
.Code to generate the authorisation header (Python)
[%collapsible]
====
[source, python]
----
...
.Code to generate the authorisation header (bash)
[%collapsible]
====
[source, bash]
----
