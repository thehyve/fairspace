...
~/bin/helm/helm fetch fairspace/fairspace --version 0.7.13
----
...
[source, shell]
----
~/bin/helm/helm install fairspace-new fairspace/fairspace --version 0.7.13 --namespace fairspace-new \
...
[source, shell]
----
~/bin/helm/helm upgrade fairspace-new fairspace-0.7.13.tgz
----
With ``helm upgrade`` you can also pass new values files with ``-f``
and pass files with ``--set-file`` as for ``helm install``.

===== Clean up deployment
To clean up an environment or completely reinstall an environment, you can use ``helm uninstall``.
